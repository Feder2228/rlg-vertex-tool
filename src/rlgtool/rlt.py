...
def closest(pixel,pal):
    """Find the closest pixel in the palette

    Args:
        pixel: the pixel
        pal: palette to search the closest pixel in
    Return:
        The closest pixel that was found
    """
    closest = 0xffffffff
...
def toU32(color):
    """Convert a color to an U32 int
    """    
    return (((int(color[2])&0xff)<<16) | ((int(color[1])&0xff) << 8) | (int(color[0])&0xff))&0xffffff




def to565(color):
    """Convert a color to 565 format
    """
...
def minmax(texel):
    """Find the biggest and the smallest color in texel
...
def encodeCMPR(ibuf,n):
    """Encode an image buffer to CMPR format

    Args:
        ibuf: the image buffer (a NumPy array)
        n: the NLGImage representing the original glt/rlt file
    """
    n.buf = []
    alpha = False
    _alpha = []
...
def to5a3From888(c, alpha):
    """Convert a something from 888 to 5a3 format

    Args:
        c: the color?
        alpha (bool): if false, the new color's alpha will always be set
            to 1 
    """
...
def encodeRGB5a3(ibuf,n):
    """Encode an image buffer to RGB5a3 format
...
def writeEmpty(o,end):
    """Write zeros to the file o, from the start to end

    Args:
        o: the file
        end: end of the section where to write zeros
    """
    while(o.tell() < end):
        o.write(b'\x00')                 
...
def decode():
    """Decode an rlt/glt image into a png image
    """
    advance(IMAGE_COUNT)
    imageCount = readInt()
...
def encodeImage(ibuf,n):
    """Encode the given NLGImage

    Use the encoding indicated in the NLGImage's "enc" field

    Args:
        ibuf: a NumPy array containing the image's data
        n: the NLGImage object
    """
    if(n.enc==0x5):
        encodeRGB5a3(ibuf,n)
    elif(n.enc==0x6):
